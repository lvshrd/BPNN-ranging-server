...
for env in envs:
    ...
    if os.path.exists(data_path):
        ...
        window_size1 = 10
        window_size2 = 50
        filtered_rssi_values1 = filter.kalman_filter(env_rssi_values,window_size1)
        filtered_rssi_values2 = filter.gaussian_filter(env_rssi_values,window_size2)
        ...
        plt.plot(filtered_rssi_values1, label="window_size ="+str(window_size1))
        plt.plot(filtered_rssi_values2, label="window_size ="+str(window_size2))
        # 添加标题和标签
        plt.title('performance of gaussian_filter')
        plt.xlabel('Index')
        plt.ylabel('Value')
        # 添加图例
        plt.legend()
